...
{
    return (IsColor(p - SG_NS - SG_WE, c) || IsColor(p - SG_NS + SG_WE, c) || IsColor(p + SG_NS - SG_WE, c) || IsColor(p + SG_NS + SG_WE, c));
}
...
inline bool GoBoard::IsLibertyOfBlock(SgPoint p, SgPoint anchor) const
{
    ...
    if (m_state.m_nuNeighbors[b->Color()][p] == 0)
        return false;
    return (m_state.m_block[p - SG_NS] == b || m_state.m_block[p - SG_WE] == b || m_state.m_block[p + SG_WE] == b || m_state.m_block[p + SG_NS] == b);
}
...
{
    return (Size() * Size() - m_state.m_numStones[SG_BLACK] - m_state.m_numStones[SG_WHITE]);
}
